...
namespace rodanit_com.yonetim
{
    public partial class manset : System.Web.UI.Page
    {
        ...
        {
            if (File.Exists(Server.MapPath("~/upload/manset/" + Class.Yonetim.Fonksiyonlar.Manset(ID).Select(p => p.dosya_ad).FirstOrDefault() + "")))
            {
                File.Delete(Server.MapPath("~/upload/manset/" + Class.Yonetim.Fonksiyonlar.Manset(ID).Select(p => p.dosya_ad).FirstOrDefault() + ""));
            }
